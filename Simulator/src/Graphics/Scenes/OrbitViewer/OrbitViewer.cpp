...
namespace Visualization
{
        ...
        : Scene(width, height)
    {
        ...
        m_Body = std::make_shared<Body>(earth, 5, width, height, "assets/Textures/earthDay.jpg");
    ...
    void OrbitViewer::OnUIRender(std::vector<ImFont *> &fonts, SimulationTime &simulationTime)
    {
        ...
        ImGui::Begin("Earth System", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar);
        ...
        propertiesSection.Heading(m_Body->GetCentralBody().getName().c_str());
        std::vector<std::string> centralRenderBodyInfo = getBodyGUIItems(m_Body);
        for (auto &item : centralRenderBodyInfo)
            propertiesSection.Text(item.c_str());

        // Central body resdolution controls
        int newResolution = m_Body->GetSubdivisionLevel();
        propertiesSection.SliderInt("Resolution", &newResolution, 2, 7, "%d");
        m_Body->changeSubdivisionLevel(newResolution);

        propertiesSection.Spacer(5.0f);

        if (propertiesSection.Button("Go To Planet Search", ImVec2(200, 40), ImVec4(0.867f, 0.345f, 0.839f, 1.0f)))
        {
        ...
        for (size_t i = 0; i < results.size(); i++)
        {
            if (results[i])
            {
                std::cout << "Selected: " << getBaseOrbitNames()[i] << std::endl;
        ...
        if (propertiesSection.Button("Go To Satellite Search", ImVec2(200, 40), ImVec4(0.867f, 0.345f, 0.839f, 1.0f)))
        {
    ...
    void OrbitViewer::Draw(SimulationTime &simulationTime)
    {
        ...
        m_Body->Draw(m_Camera->GetCameraInfo(), glm::vec2(m_Width / 2, m_Height / 2), m_imageBuffer);
    ...
    void OrbitViewer::updateSatelliteData(const std::vector<SatelliteData> &data)
    {
        m_Orbits.clear();
        OrbitalObject moon = OrbitalObjectBuilder("Moon", Type::Moon, 7.34767309e22)
                                 .setSemiMajorAxis(384400.0)
                                 ...
                                 .setInclination(5.145)
                                 .setLongitudeOfAscendingNode(125.08)
                                 .setCentralBody(m_Body->GetCentralBody())
                                 .build();

        m_Orbits.insert(std::make_pair(moon.getName(), std::make_shared<Orbit>(moon, m_baseOrbitColor, m_baseIconColor, m_Width, m_Height)));

        for (auto satellite : data)
        {
            OrbitalObject orbit = OrbitalObjectBuilder(satellite.name, Type::Satellite, 100.0)
                                      .setSemiMajorAxis(satellite.semiMajorAxis)
                                      .setEccentricity(satellite.eccentricity)
                                      .setInclination(satellite.inclination)
                                      .setLongitudeOfAscendingNode(satellite.longitudeOfAscendingNode)
                                      .setCentralBody(m_Body->GetCentralBody())
                                      .build();

            m_Orbits.insert(std::make_pair(satellite.name, std::make_shared<Orbit>(orbit, m_baseOrbitColor, m_baseIconColor, m_Width, m_Height)));
        }

        setTargetOrbit("Moon");
    }

    void OrbitViewer::updatePlanetData(const PlanetData& planetData)
    {
        Type type = Type::Exoplanet;
        if (planetData.isSolarSystem)
        {
            type = Type::Planet;
        }
        CentralBody newBody(planetData.name, type, planetData.mass, planetData.radius, 3.0);
        // free the current m_Body
        m_Body.reset();

        m_Body = std::make_shared<Body>(newBody, 5, m_Width, m_Height, planetData.texturePath);

        // TODO update orbits
        std::unordered_map<std::string, std::shared_ptr<Orbit>> orbitsCopy = m_Orbits;
        m_Orbits.clear();
        OrbitalObject moon = OrbitalObjectBuilder("Moon", Type::Moon, 7.34767309e22)
                                 .setSemiMajorAxis(384400.0)
                                 .setEccentricity(0.0549)
        ...
        for (auto orbit : orbitsCopy)
        {
            double semiMajorAxis = orbit.second->GetOrbitalObject().getSemiMajorAxis();
            double eccentricity = orbit.second->GetOrbitalObject().getEccentricity();
            double inclination = orbit.second->GetOrbitalObject().getInclination();
            double longitudeOfAscendingNode = orbit.second->GetOrbitalObject().getLongitudeOfAscendingNode();
            OrbitalObject newOrbit = OrbitalObjectBuilder(orbit.first, Type::Satellite, 100.0)
                                         .setSemiMajorAxis(semiMajorAxis)
                                         .setEccentricity(eccentricity)
                                         .setInclination(inclination)
                                         .setLongitudeOfAscendingNode(longitudeOfAscendingNode)
                                         .setCentralBody(m_Body->GetCentralBody())
                                         .build();
            m_Orbits.insert(std::make_pair(orbit.first, std::make_shared<Orbit>(newOrbit, m_baseOrbitColor, m_baseIconColor, m_Width, m_Height)));
        }
